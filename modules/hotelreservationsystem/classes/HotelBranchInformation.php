...
class HotelBranchInformation extends ObjectModel
{
    ...
    public function hotelBranchesInfo($idLang = false, $active = 2, $detailedInfo = 0, $idHotel = 0)
    {
        ...
        if ($detailedInfo) {
            $sql .= ', hi.id as id_cover_img,hi.hotel_image_id,s.`name` as `state_name`, cl.`name` as country_name';
        }
    ...
    public function getHotelImages($idHotel){
        error_log('ggggggggggggggggggggggggggggggggggggggggggggggggggggg');
        $sql ='SELECT hi.* FROM `'._DB_PREFIX_.'htl_image` hi WHERE hi.`id_hotel`='.(int)$idHotel;
    ...
    public function hotelBranchInfoByCategoryId($cat_id)
    {
        ...
        error_log('######################## hotelBranchInfoByCategoryId ###################');
        $idLang = Context::getContext()->language->id;
        $sql = 'SELECT hbi.`id`, hbi.`id_category`, hbl.`hotel_name`, hbl.`description` FROM `'._DB_PREFIX_.'htl_branch_info` hbi
            LEFT JOIN `'._DB_PREFIX_.'htl_branch_info_lang` hbl
            ON (hbl.`id` = hbi.`id` AND hbl.`id_lang` = '.(int)$idLang.')
    ...
    public function hotelBranchInfoAndHotelImagesByCategoryId($cat_id)
    {
        error_log('######################## hotelBranchInfoByCategoryId ###################');
        error_log(print_r($cat_id,true));
        error_log('######################## hotelBranchInfoByCategoryId ###################');
        $idLang = Context::getContext()->language->id;
        $sql = 'SELECT hbi.`id`, hbi.`id_category`, hbl.`hotel_name`, hbl.`description`, hi.`hotel_image_id` FROM `'._DB_PREFIX_.'htl_branch_info` hbi
            LEFT JOIN `'._DB_PREFIX_.'htl_branch_info_lang` hbl
            ON (hbl.`id` = hbi.`id` AND hbl.`id_lang` = '.(int)$idLang.')
            INNER JOIN '._DB_PREFIX_.'htl_image hi
            ON hbi.id = hi.id_hotel
            WHERE hbi.`id_category` = '.(int)$cat_id.' AND `active` = 1';
